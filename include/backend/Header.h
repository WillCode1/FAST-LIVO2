...
void state2pose(PointXYZIRPYT &this_pose6d, const double &lidar_end_time, const ikfom_state &state, const M3D &ext_r, const V3D &ext_t)
{
    // imu pose -> lidar pose
    M3D lidar_rot;
    V3D lidar_pos;
    poseTransformFrame(state.rot_end, state.pos_end, ext_r, ext_t, lidar_rot, lidar_pos);
...
template <typename ikfom_state>
void pose2state(const PointXYZIRPYT &this_pose6d, ikfom_state &state, const M3D &ext_r, const V3D &ext_t)
{
    // lidar pose -> imu pose
    ...
    poseTransformFrame2(lidar_rot, lidar_pos, ext_r, ext_t, state.rot_end, state.pos_end);
}
